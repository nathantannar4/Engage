...
class CreateSimpleEventViewController: FormViewController, UserSelectionDelegate {
    ...
    override func viewDidLoad() {
        ...
        let object = PFObject(className: Engagement.current().queryName! + PF_EVENT_CLASS_NAME)
        self.event = Event(fromObject: object)
        
        self.setTitleView(title: "Event", subtitle: "Create", titleColor: Color.defaultTitle, subtitleColor: Color.defaultSubtitle)
    ...
    override func viewDidAppear(_ animated: Bool) {
        self.locationRow.cellUpdate { (cell) in
            cell.titleLabel.text = self.event.location
        }
    }
    ...
    private lazy var locationRow: CustomRowFormer<TitleCell> = {
        CustomRowFormer<TitleCell>(instantiateType: .Nib(nibName: "TitleCell")) {
            $0.titleLabel.text = self.event.location
    ...
    private func configure() {
        ...
        let endRow = InlineDatePickerRowFormer<FormInlineDatePickerCell>() {
            ...
            $0.displayLabel.font = RobotoFont.regular(with: 15)
                ...
                $0.date = self.event.start ?? Date()
            }.inlineCellSetup {
                $0.datePicker.datePickerMode = .dateAndTime
                $0.datePicker.date = self.event.start
            }.onDateChanged {
                self.event.end = $0 as NSDate!
        ...
        let startRow = InlineDatePickerRowFormer<FormInlineDatePickerCell>() {
            ...
            $0.displayLabel.font = RobotoFont.regular(with: 15)
                ...
                $0.date = self.event.start
            }.inlineCellSetup {
                $0.datePicker.datePickerMode = .dateAndTime
            }.onDateChanged {
                self.event.start = $0
                if (self.event.start?.isLaterThanDate(self.event.end as! Date))! {
                    endRow.update {
                        $0.date = self.event.start
                    }
                }
        ...
        let allDayRow = SwitchRowFormer<FormSwitchCell>() {
            ...
            $0.titleLabel.textColor = Color.defaultNavbarTint
            $0.titleLabel.font = RobotoFont.medium(with: 15)
            $0.switchButton.onTintColor = Color.defaultNavbarTint
            }.onSwitchChanged { on in
                self.event.allDay = on
                startRow.update {
    ...
    func didSelectMultipleUsers(selectedUsers: [PFUser]!) {
        ...
        self.event.inviteTo = selectedUsers
